...
public:
    ...
    pthread_t rotate_nozzle_thread;
    void *rotate_nozzle();
    ...
    std::vector <trajectory_msgs::JointTrajectoryPoint> combine_trajectory(float speed_factor);

    std::vector <trajectory_msgs::JointTrajectoryPoint> compute_linear_trajectory(trajectory_msgs::JointTrajectoryPoint,
                                                                           geometry_msgs::Pose, float);

    std::vector <trajectory_msgs::JointTrajectoryPoint> compute_joint_trajectory(std::vector<double> initial_angles,
    ...
    void vacuum_suction_on_off(ros::ServiceClient&, bool, int, int, int);

    bool read_joson_file();

    bool write_jason_file(int bin_number);

    bool move_towards_centroid_via_normal_mohit();

    std::vector <trajectory_msgs::JointTrajectoryPoint> move_towards_centroid_via_normal_mohit2();

    std::vector <trajectory_msgs::JointTrajectoryPoint> compute_linear_trajectory2(std::vector<double> initial_angles,
                                                                                 std::vector<double> initial_velocities,
                                                                                 geometry_msgs::Pose final_pose,
                                                                                 float speed_factor);

    std::vector <trajectory_msgs::JointTrajectoryPoint> reach_bin2(double location[0]);

    std::vector <trajectory_msgs::JointTrajectoryPoint> combine_trajectory2( float speed_factor, double location[0]);

    bool reach_bin3(double location[0]);

    std::vector <trajectory_msgs::JointTrajectoryPoint>  compute_trajectory_from_tote_to_object();

    std::vector <trajectory_msgs::JointTrajectoryPoint>  compute_trajectory_from_object_to_bin(double location[0]);

    std::vector <trajectory_msgs::JointTrajectoryPoint>  compute_trajectory_from_bin_to_tote();

    bool suck_object();

    bool put_object(double location[0]);

    bool go_to_tote();

    bool publish_nozzle_angles(int real, double angle);
