...
public:
    ...
    bool checkProtectiveStopCallback(apc_nozel::CheckClearProtectiveStop::Request &req, apc_nozel::CheckClearProtectiveStop::Response &res)
    {
        if (robot_.sec_interface_->robot_state_->isProtectiveStopped())
        {
            print_error("Check PS: Robot is protective stopped!");
...
private:
	...
	void goalCB(
        ...
        if (!start_positions_match(goal.trajectory, 0.05)) {
			result_.error_code = result_.INVALID_GOAL;
...
int main(int argc, char **argv) {
    ...
    ros::ServiceServer service_check_protective_stop = nh.advertiseService<apc_nozel::CheckClearProtectiveStop::Request, apc_nozel::CheckClearProtectiveStop::Response>
            ("/iitktcs/ur_modern_driver/check_protective_stop",
             boost::bind(&RosWrapper::checkProtectiveStopCallback, &interface, _1, _2));
